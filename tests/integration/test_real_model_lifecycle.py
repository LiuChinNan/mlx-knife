...
class TestRealModelLifecycle:
    ...
    def setup_isolated_model(self, class_temp_cache_dir):
        ...
        try:
            ...
            print("🔒 Using isolated test cache (user cache untouched)")
    ...
    def test_02_list_shows_downloaded_model(self):
        ...
        print("✓ Model appears in list output with health status")
    ...
    def test_03_show_detects_real_framework(self):
        ...
        print("✓ Show command detected framework and health for real model")
    ...
    def test_04_find_matching_works_with_real_model(self):
        ...
        assert len(partial_matches) >= 1, "Partial match failed for 'tiny-random'"
    ...
    def test_05_resolve_real_model_paths(self):
        ...
        assert commit_hash is not None, "Commit hash not resolved"
        assert len(commit_hash) >= 8, f"Invalid commit hash: {commit_hash}"

        # Test fuzzy resolution
        fuzzy_path, fuzzy_name, fuzzy_hash = resolve_single_model("tiny-random")

        assert fuzzy_path is not None, "Fuzzy resolution failed for 'tiny-random'"
    ...
    def test_06_health_check_on_real_model(self):
        ...
        assert framework is not None, "Framework detection failed for real model"
        ...
        print("✓ Show command also reports health status correctly")
    ...
    def test_07_rm_cleans_locks_and_model(self):
        ...
        print("✓ rm command cleaned both model and locks (Issue #23 fix verified)")
    ...
    def test_08_model_completely_removed(self):
        ...
        print("✓ Model completely removed from cache and indexes")
